...
export const skill_info_get_single_controller = async (
	req: Request,
	res: Response
) => {
	if (req.user && req.params) {
		...
		try {
			const skill_info_found: skill = await prisma.skill.findUniqueOrThrow({
				where: {
					id,
				},
			});
			...
			return res.status(200).json(skill_info_found);
		} catch (error) {
